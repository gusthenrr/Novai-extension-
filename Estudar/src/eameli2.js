...
var nvaiProxy = getRandomProxy()
var nvaiProxyLessRestricted = "https://mfy.herokuapp.com/";
...
var nvai_version = typeof nvai_version !== 'undefined' ? nvai_version : null;
...
<div data-nvai-spinner="true" role="status" aria-live="polite" class="nvai-atom-wrap">
  ...
  // Detecta loader NVAI (e também legados nvai/lottie pra evitar duplicar)
  hasSpinner(node) {
    if (!node) return false;
    return !!node.querySelector('[data-nvai-spinner],[data-novai-spinner],[data-nvai-spinner],lottie-player');
  }
...
let _nvaiKeepAliveInterval = null;
let _nvaiKeepAliveObserver = null;
let _nvaiLastReinitAt = 0;
const _nvaiMinReinitGapMs = 1200;
function _nvaiScheduleReinit(reason) {
  const now = Date.now();
  if (now - _nvaiLastReinitAt < _nvaiMinReinitGapMs) return;
  _nvaiLastReinitAt = now;
  try { initializeExtensionFeatures(); } catch (_) {}
}
function _nvaiKeepAliveTick() {
  try {
    if (paginaAtual === 'anuncio') {
      const hasSwitch = document.getElementById('eaoffSwitch');
      if (!hasSwitch) return _nvaiScheduleReinit('missing eaoffSwitch');
    } else if (paginaAtual === 'lista') {
      const hasCTA = document.getElementById('ealistrequest') || document.getElementById('nvai-catalog-filter-container');
      if (!hasCTA) return _nvaiScheduleReinit('missing list widgets');
    }
  } catch (_) {}
}
function startKeepAlive() {
  if (!_nvaiKeepAliveInterval) {
    _nvaiKeepAliveInterval = setInterval(_nvaiKeepAliveTick, 1500);
  }
  if (!_nvaiKeepAliveObserver && typeof MutationObserver !== 'undefined') {
    const root = document.getElementById('root-app') || document.body;
    _nvaiKeepAliveObserver = new MutationObserver((mutations) => {
      ...
      if (removedOurNode) setTimeout(_nvaiKeepAliveTick, 100);
    });
    try { _nvaiKeepAliveObserver.observe(root, { childList: true, subtree: true }); } catch (_) {}
  }
}
function parseJwt(e) {
...
var nvaiMainColor = typeof nvaiMainColor === "undefined" ? "#7933ff" : nvaiMainColor;
var nvaiMainColorNumbers = typeof nvaiMainColorNumbers === "undefined" ? (typeof nvaiMainColor === "string" ? nvaiMainColor.replace('#','') : "7933ff") : nvaiMainColorNumbers;
isList();
...
var toolModal = `
  <div id="nvai-tool-modal" class="toolmodal">
    <div class="andes-modal__portal" style="display:none" id="nvai-modal-portal">
      <div id="nvai-modal-overlay" class="andes-modal__overlay andes-modal__overlay--small">
        <div role="dialog" tabindex="-1" class="andes-modal ui-pdp-iframe-reviews andes-modal--small" data-ismodal="true" aria-modal="true" style="max-height: 100%; max-width: 100%;">
          ...
          <div class="andes-modal__scroll">
            <div class="andes-modal__content" id="nvai-modal-content" style="padding: 4rem;display: flex;flex-direction: column;justify-content: center;align-items: center; min-width: 51rem;">
              ...
              <div id="nvai-track-chart" style="width: 59rem;height: 27rem;padding-top: 2.1rem;min-height: 365px;">Gráfico</div>
            </div>
          </div>
        </div>
      </div>
            ...
            <div class="inner-column-1" style="display: flex;flex-direction: row;flex: 1;">
              ...
              <div style="display: flex;flex-direction: column;">
                <div style="display: flex;margin-bottom: 7px;">
                  <span class="codefield" style="pointer-events: all;font-weight: 700;text-align: left;font-size: 1.35em;padding: 0.75em 1em;border: 1px solid var(--nvai-main)54;border-radius: 1em;">0000000000000</span>
                </div>
...
<br />
<span id="mediabtn" class="andes-button--loud nvai-main-bg andes-button" style="margin-top: 0.35em;font-size: 12px!important;display:inline!important;padding-top: 1em;padding-bottom: 1em;position: relative;z-index: 10;border-radius:2rem;">
...
var eanotifytag = `
<div style="background-color: var(--nvai-main);background-image: linear-gradient(to right, #003eba -42%, var(--nvai-main) 35%);width:2em;height:2em;position: absolute;top: 2.7em;right: -1em;cursor: pointer;border-right: 2px solid #2c4cff;border-radius: 0px 5px 5px 0px;box-shadow: 0.31em 0 0.35em -0.35em rgb(0 0 0);">
...
<div id="eameter_modal" class="andes-tooltip andes-tooltip--light" style="margin-bottom: -2em;box-shadow: rgb(0 0 0 / 21%) 0px 3px 6px;position: relative;top: -3em;opacity: 0;transition: opacity 0.35s ease 0s;background-color: rgb(255, 255, 255);border-radius: 0.35em;text-align: left;display: none;width: 13em;padding: 3em 1em 1em 1em;">
  <span style=" font-size: 1em; font-weight: 600; color: var(--nvai-main);">Saúde do Anúncio</span><br>
...
<span id="eagrossrev">
  ...
  <div class="earevstats">
    <span style="font-size: 11px;" class="ui-pdp-review__amount" id="nvai_rev_estimate">Média de faturamento estimada. </span><br>
    <button class="andes-button--loud nvai-main-bg  revbtn1"  style="padding: .1em .5em;border-radius: 5px;margin: 2px;">1 Dia</button>
    <button class="andes-button--loud nvai-main-bg  revbtn7"  style="padding: .1em .5em;border-radius: 5px;margin: 2px;">7 Dias</button>
    <button class="andes-button--loud nvai-main-bg  revbtn30" style="padding: .1em .5em;border-radius: 5px;margin: 2px;">30 dias</button>
    <button class="andes-button--loud nvai-main-bg  revbtn60" style="padding: .1em .5em;border-radius: 5px;margin: 2px;">60 dias</button>
    <button class="andes-button--loud nvai-main-bg  revbtn90" style="padding: .1em .5em;border-radius: 5px;margin: 2px;">90 dias</button>
    <button class="andes-button--loud nvai-main-bg  revbtntotal" style="padding: .1em .5em;border-radius: 5px;margin: 2px;">Total</button>
  </div>
</span>
`;
...
var btn_preco = `
<div id="preco-btn" class="andes-button andes-button--loud nvai-main-bg pricebtn"
  ...
  if (1 == eapricewarning) {
    (function () {
      ...
      (async function (t) {
        ...
        s.total_items_in_this_category && n ? n.innerHTML = `<b style="color: var(--nvai-main);font-size:18px;">${s.total_items_in_this_category}</b> anúncios na categoria.`: n.parentElement.parentElement.remove();
      })(e);
    })();
  }
}
...
async function fetchProductDataFromPage(e, t) {
  ...
  if (iscatalog = !0, itemsLocalData[e] || (document.dispatchEvent(new CustomEvent("GetProductData", {
    ...
    if (a && vendas > 0) {
      let e = a.innerHTML;
      a.parentElement.style.margin = "1rem 0", a.innerHTML = e + ' (no catálogo)<br><strong style="font-weight: 900;color: var(--nvai-main);">' + vendas + ' vendas</strong><span style="font-size: 0.77em;position: relative;top: -0.1em;"> (deste modelo &amp; vendedor)</span>', a.setAttribute("sales", vendas)
    }
    t()
  }
  else {
        ...
        if (a) t();
          ...
          if (s.length > 0) {
            ...
            if (o > 0 && null != a) {
              ...
              if (e) {
                let t = e.innerHTML;
                e.parentElement.style.margin = "1rem 0", e.innerHTML = t + ' (no catálogo)<br><strong style="font-weight: 900;color: var(--nvai-main);">' + vendas + ' vendas</strong><span style="font-size: 0.77em;position: relative;top: -0.1em;"> (deste modelo &amp; vendedor)</span>', e.setAttribute("sales", vendas)
              }
              let t = document.getElementById("mediabtn");
              if (t && dias > 0) {
                let e = `
<span class="nvai-info-icon_catalog-sales" style="margin: 0 -0.75rem 0 0.75rem;cursor:pointer;">
...
`;
                ...
                let a = document.getElementsByClassName("nvai-catalog-info-tooltip")[0];
...
function buildMainComponentSkeleton() {
 return `
  <div id="main-component-skeleton" class="nvai-skel-root">
    <style>
      .nvai-skel-root{
      ...
      .skeleton-pill{
        ...
        animation: nvai-loading 1.5s infinite;
        color: transparent;
      ...
      /* layout */
      .nvai-row{ display:flex; align-items:center; justify-content:space-between; margin-bottom:1rem; padding:0 2rem 0 0; }
      .nvai-left{ display:flex; align-items:center; }
      .nvai-avatar{
        width:40px; height:40px; background-color: var(--nvai-main);
        border-radius:50%; display:flex; align-items:center; justify-content:center; margin-right:12px;
      }
      .nvai-left .nvai-title { font-size:15px; }
      .nvai-left .nvai-conv { font-size:14px; padding:4px 12px; margin-top:4px; }

      .nvai-right{ text-align:right; }
      .nvai-right .nvai-label{ font-size:14px; }
      .nvai-right .nvai-value{ font-size:16px; font-weight:bold; color: var(--nvai-main); }
      ...
      .nvai-rev-card .nvai-rev-text{ font-size:1.1em; font-weight:900; flex:1; }
      .nvai-dropdown{
      ...
      .nvai-footer{ display:flex; align-items:center; justify-content:space-between; }
      .nvai-footer .nvai-created{
      ...
      .nvai-footer .nvai-avg{ background-color:#3483fa; color:transparent; padding:10px 20px; font-size:16px; font-weight:bold; }
    </style>

    <div class="nvai-row">
      <div class="nvai-left">
        <div class="nvai-avatar">
          <img src="https://img.icons8.com/ios-glyphs/32/ffffff/combo-chart.png" style="width:1.35em; margin:auto;">
        </div>
        <div>
          <div class="nvai-title"><span class="skeleton">30884</span> Visitas totais</div>
          <div class="skeleton-pill nvai-conv">Conversão: 3.2%</div>
        </div>
      </div>

      <div class="nvai-right">
        <div class="nvai-label">Vende a cada:</div>
        <div class="nvai-value"><span class="skeleton">000</span> Visitas</div>
      </div>
    </div>

    <div class="nvai-rev-card">
      <img src="https://img.icons8.com/windows/32/c7c7c7/old-cash-register.png"
           style="width:1.5em; height:1.5em; position:relative; top:.21em; margin-right:.5em;">
      <div class="nvai-rev-text">
        Faturando: <strong style="font-size:15px;"><span class="skeleton">R$ 1.932,56</span>/mês</strong>
      </div>
      <div class="nvai-dropdown">
    ...
    <div class="nvai-footer">
      <div class="nvai-created">Criado há: <span class="skeleton">520</span> dia(s)</div>
      <div class="skeleton-pill nvai-avg">Média: 58 vendas/mês</div>
    </div>
  </div>
`;
    ...
    let t = setInterval((function () {
      if (null != document.getElementsByClassName("andes-form-control__label")[0]) {
        ...
        n++) if ("Código universal de produto" == e[n].innerHTML) {
          let a = '<div class="eangen" style="padding: 0em 0.5em;background: var(--nvai-main);border-radius: 1em;margin: 1.5em 1em;width: 5em;height: 5em;text-align: center;cursor: pointer;transition: all 0.35s;transform: scale(1);"> <div style="width: 2.1em;height: 2em;margin-bottom: -2em;background-color: var(--nvai-main);position: relative;top: -1em;left: -1.1em;display: flex;border-radius: 10em;"> <img src="https://i.ibb.co/7tHHPPY/icon-color.png" style="pointer-events:none;width:15px;margin: auto;filter: brightness(7.5);"></div> <img src="https://img.icons8.com/carbon-copy/100/ffffff/refresh-barcode.png" style="margin: auto;width: 100%;margin-bottom: -0.5em;"><span style=" /* margin: auto; */ font-size: 11px; color: var(--nvai-main); font-weight: 900;">Gerar EAN13</span> </div>';
        ...
        e++) if ("Código universal de produto (opcional)" == n[e].innerHTML) {
          let a = '<div class="eangen" style="display: flex;background: var(--nvai-main);border-radius: 1em;margin: 1em 1em 0em 1em;width: 3em;height: 3em;text-align: left;cursor: pointer;transition: all 0.35s;transform: scale(1);"> <div style="width: 2.1em;height: 2em;margin-bottom: -2em;background-color: var(--nvai-main);position: relative;top: 0.5em;left: -1.5em;display: flex;padding: 0em 0.35em;border-radius: 10em;"> <img src="https://i.ibb.co/7tHHPPY/icon-color.png" style="pointer-events:none;width:15px;margin: auto;filter: brightness(7.5);"></div> <img src="https://img.icons8.com/carbon-copy/100/ffffff/refresh-barcode.png" style="display: flex;width: 3em;margin-left: -1.75em;"><span style="display: flex;margin: auto;font-size: 12px;line-height: 1em;padding-left: 0.4em;color: var(--nvai-main);font-weight: 900;">Gerar EAN13</span> </div>', i = n[e].parentNode.parentNode.parentNode.parentNode.getElementsByTagName("tbody")[0].getElementsByTagName("input");
          for (let e = 0;
          e < i.length;
...
async function findDocID(e, t, n) {
  console.log("Buscando documento de identidade...", nvaiProxy);
  t = t || 0, await fetch(`${nvaiProxy}https://api.mercadolibre.com/users/me`, eaInit).then((e => e.json())).then((e => rawID = e.identification.number ?? void 0)).catch ((function (e) {})), generateEAN13(e, t, !!n)
}
function generateEAN13(e, t, n) {
  ...
  if (n) {
    ...
    t.innerHTML = o, t.setAttribute("style", "font-weight: 700; font-size: 1.1em; color: var(--nvai-main);width:16em;")
  }
  else document.getElementsByClassName("codefield")[0] ? document.getElementsByClassName("codefield")[0].innerHTML = o: document.getElementsByClassName("eangen")[0].parentElement.lastChild.innerHTML = '<span style=" font-weight: 700; font-size: 1.1em; color: var(--nvai-main);">' + o + "</span>";
  var r;
...
function fetchCategoryWithCache(e, t) {
  ...
  const n = a => {
    ...
    = a.detail;
    i === e && (s ? t(s): fetch(`${nvaiProxy}https://api.mercadolibre.com/categories/${e}`, eaInit).then((e => e.json())).then((n => {
  ...
  function t() {
    ...
    function n() {
      ...
      e.addEventListener("click", (function (e) {
        ...
        let b = document.getElementById("nvai_rev_estimate");
    ...
    function a() {
      ...
      function t(t) {
        if (null == t) {
          ...
          i?.addEventListener("click", (function () {
            "true" == this.getAttribute("open") ? (s.style.opacity = "0", s.style.height = "0px", s.style.margin = "-3em 0em 0em 0em", this.setAttribute("open", "false")): "false" == this.getAttribute("open") && (s.style.opacity = "1", s.style.height = "auto", s.style.margin = "-3em 0em 2em 0em", this.setAttribute("open", "true")), "none" == this.getAttribute("sellerdata") && fetch(`${nvaiProxy}https://api.mercadolibre.com/users/${vendedor}`, eaInit).then((t => e(t))).catch ((function (e) {}))
          }
          ))
        }
        else {
          ...
          function n(n) {
            l += n, function (n) {
              ...
              for (let e = 0;
              e < u.length;
              e++) y += `<span style="padding: 1px 7px;border-radius:11px;font-weight: 400;font-size:12px;margin: 2px 2px;" class="andes-button--loud nvai-main-bg ">${u[e]}</span>`;
              let h = t.seller.seller_reputation.transactions.canceled, b = h / m * 100, v = `<span style="font-weight: 900;font-size: 0.92em;display: block;margin: 4px;">${r} Anúncios <span style="padding: 1px 7px;border-radius:11px;font-weight: 400;font-size:12px;margin: 2px 2px;border: 2px solid #ebebeb;">até R$79</span></span>\n\n                        <span style="font-weight: 400;font-size: 0.92em;display: block;margin: 4px;">${l} Anúncios <span style="padding: 1px 7px;border-radius:11px;font-weight: 400;font-size:12px;margin: 2px 2px;border: 2px solid #ebebeb;">R$80 até R$150</span></span>\n\n                        <span style="font-weight: 400;font-size: 0.92em;display: block;margin: 4px;">${d} Anúncios <span style="padding: 1px 7px;border-radius:11px;font-weight: 400;font-size:12px;margin: 2px 2px;border: 2px solid #ebebeb;">Acima de R$150</span></span>`, x = `<div class="ealine" style="display: flex;flex-wrap: wrap;"><img src="https://img.icons8.com/fluency-systems-regular/96/${nvaiMainColorNumbers}/shop-location.png" style="width: 2.7em;margin: 0px 7px 7px 7px;"> <div style="display: grid;"><span style="font-size: 1.1em;font-weight: 700;">${t.seller.nickname}</span><span style="font-size: 11px;margin-top: -0.75em;">Conta criada: ${i}<span class="andes-button--loud nvai-main-bg " style="margin-left: 0.5em;padding: 1px 5px;border-radius: 11px;font-weight: 900;font-size: 10px;">${s}</span></span></div></div>\n\n                        <div style="width: 100%;background: #ebebeb;height: 1px;"></div>\n                        \n                        <div class="ealine" style="display: flex;flex-wrap: wrap;margin-top: 0.5em;"> <div style="padding-left: 1em;"><span style="font-size: 1em;font-weight: 900;">Vendas totais: </span>${m} <span style="padding: 1px 7px;border-radius: 11px;font-weight: 400;font-size: 10px;background-color: #a3a3a3;" class="andes-button--loud nvai-main-bg ">${h} canceladas (${b.toFixed(1)}%)</span></div></div>\n                        \n                        <div class="ealine" style="display: flex;flex-wrap: wrap;"> <div style="padding-left: 1em;margin: 4px 0px;"><span style="font-size: 1em;font-weight: 900;">Anúncios:</span> ${t.paging.total} <span style="padding: 1px 7px;border-radius: 11px;font-weight: 400;font-size: 10px;background-color: #39b54a;margin: 0px 4px;" class="andes-button--loud nvai-main-bg ">${o} <svg xmlns="http://www.w3.org/2000/svg" class="logo-full" width="151" height="39" viewBox="0 0 151 39" data-reactroot="" style="width: 3.75em;height: auto;position: relative;top: 0.2em;padding: 0em 0em 0em 0.35em;"><g fill="#ffffff" fill-rule="evenodd"><path d="M9.577 0L0 22.286h15.962L9.577 39l25.54-25.071H19.153L28.732 0zM56.094 27.925h-6.931l5.924-24.38h19.706l-1.33 5.483H60.688l-.886 3.801h12.452l-1.33 5.483H58.433l-2.338 9.613zm33.718.439c-8.262 0-12.332-3.582-12.332-8.7 0-.402.12-1.242.202-1.608l3.546-14.51h7.052L84.774 17.91c-.04.183-.12.585-.12 1.023.04 2.01 1.732 3.948 5.158 3.948 3.707 0 5.601-2.12 6.286-4.971l3.507-14.365h7.012L103.11 18.02c-1.451 5.921-4.998 10.344-13.3 10.344zm36.014-.439h-17.732l5.924-24.38h6.932l-4.554 18.897h10.76l-1.33 5.483zm23.844 0h-17.732l5.924-24.38h6.932l-4.554 18.897H151l-1.33 5.483z"></path></g></svg></span><span style="padding: 1px 7px;border-radius: 11px;font-weight: 400;font-size: 10px;background-color: #39b54a;" class="andes-button--loud nvai-main-bg ">${a} Frete Grátis</span></div></div>\n\n                        <div class="ealine" style="display: flex;flex-wrap: wrap;"> <div style="padding-left: 1em;margin: 4px 0px;"><span style="font-size: 1em;font-weight: 900;">Anúncios por Ticket:</span><div style="border-left: 2px solid var(--nvai-main);">${v}</div></div></div>\n                        \n                        <div class="ealine" style="display: flex;flex-wrap: wrap;"> <div style="padding-left: 1em;display: flex;flex-wrap: wrap;"><span style="font-size: 1em;font-weight: 900;">Categorias principais do vendedor: </span>${y}</div></div>`;
          ...
          (function () {
            ...
            for (let t = 0;
            t < c;
            t++) e.push(fetch(`${nvaiProxy}https://api.mercadolibre.com/sites/MLB/search?seller_id=${vendedor}&offset=${50*t}`, eaInit));
    ...
    function i() {
      document.getElementsByClassName("ui-pdp-breadcrumb")[0].insertAdjacentHTML("beforeend", '<div id="eacattrends" style="width:62em"><span id="eacattrendsbtn" style="font-weight:700;background-color:var(--nvai-main);color:#fff;padding:0.35em 0.75em;border-radius:7px;margin: 0em 0.5em;cursor: pointer;position: relative;top: -4px;z-index: 3;"><img src="https://img.icons8.com/ios-glyphs/60/ffffff/hot-sales-hours.png" style="width: 1.21em;position: relative;top: 3px;">\n            <span>Termos mais buscados! </span> <span style="font-size: 0.7em;position: relative;top: -2px;right: -3px;padding: 0px 5px 1px 5px;margin: 0px 0px 0px 5px;border: 1px solid #fff;border-radius: 1em;">categoria</span></span></div>');
      let e = document.getElementById("eacattrends");
      function t() {
        n.getElementsByTagName("span")[0].innerText = "Carregando...", fetch(`${nvaiProxy}https://api.mercadolibre.com/trends/MLB/${categoria_Local}`, eaInit).then((e => e.json())).then((e => function (e) {
          let n = e, a = [];
          for (let e = 0;
          e < n.length;
...
const t = `\n      <div id="nvai-admarker" style="font-size: 0.95rem;display: inline-flex;border-radius: 1em;color: #5a5a5a;box-shadow: 0px 2px 11px -7px #000;padding: 0.2em 1.2em 0.2em 0.2em;align-items: center;width: fit-content;height: 1.5rem;" class="ui-pdp-review__amount">\n        <svg class="ui-pdp-icon ui-pdp-icon--protected ui-pdp-color--GRAY" style="width: 1rem; margin: -2px 7px;" xmlns="http://www.w3.org/2000/svg" width="12" height="14" viewBox="0 0 12 14">\n          <use href="#warranty"></use>\n        </svg>\n        ${e}\n      </div>`;
...
!function () {
  if ("anuncio" == paginaAtual) {
    ...
    btn_preco_fix =
    '<div id="preco-btn" class="andes-button andes-button--loud nvai-main-bg  pricebtn" style="width: 3em;height: 3em;margin-top: 1em;margin-right: 0.5em;padding:0.5em 0.1em 1em 0.1em;border-radius: 3.5em;position: relative;z-index: 100;"><img id="preco-img" style="width:50%;" src="https://img.icons8.com/ios-glyphs/30/ffffff/estimate.png"/></div>',
  iscatalog
) {
    ...
    function o() {
      if (null != verif && "pro" == verif) {
        ...
        const b = h.map((e => dayjs(e).endOf("month").format("YYYY-MM-DD"))), v = [...h.map(((e, t) => `${nvaiProxyLessRestricted}https://api.mercadolibre.com/items/visits?ids=${item_ID}&date_from=${e}&date_to=${b[t]}`))], x = [];
        function o(e, t) {
          let n = document.getElementById("eagraph"), a = document.getElementById("salesestimatebtn");
          if (n) {
            n.innerHTML = '<span style="font-size: 1.5em;margin-left: 0.35em;font-weight: 700;color: #333;">Análise de Interesse</span> <br> <span style="color: var(--nvai-main);font-size: 1.35em;position: relative;top: -0.31em;margin-left: 0.35em;">dos últimos 6 meses</span><br><div id="eachart" style="width:100%;position: relative;top: -1em;left: -0.7em;"></div><button id="salesestimatebtn" class="andes-button--loud nvai-main-bg " style="margin-top: -1em;font-size: 1.1rem;width: 100%;font-weight: 800;padding: 7px 11px;border-radius: 7px;">Estimativa de vendas</button>', n.style.top = "2rem", document.getElementById("salesestimatebtn").remove(), n.insertAdjacentElement("beforeend", a), document.getElementById("salesestimatebtn").style.display = "none", document.getElementById("eachart").style.marginBottom = "-1.5em";
        ...
        const T = e => {
          ...
          = e.detail;
          t === item_ID && n ? d(n): fetch(`${nvaiProxyLessRestricted}https://api.mercadolibre.com/visits/items?ids=${item_ID}`, eaInit).then((e => e.json())).then((e => {
        ...
        (), t(), async function () {
          ...
          const e = t => {
            ...
            = t.detail;
            n === categoria_Local && a && a.listing ? m(a.listing): fetch(`${nvaiProxyLessRestricted}https://api.mercadolibre.com/sites/MLB/listing_prices?price=${preco_Local}&category_id=${categoria_Local}&listing_type_id=${tipo_anuncio}`, eaInit).then((e => e.json())).then((e => {
              if (a) {
                const t = {
                  ...a,
                  listing: e
...
<div id="ealistrequest"
  ...
  <svg aria-hidden="true" viewBox="0 0 24 24" width="20" height="20"
    ...
    let u = `
    <div class="nvai-ad-listinfo_widget" style="display: flex;align-items: center;justify-content: space-between;border-radius: 5rem;margin: 0rem 1rem;position: relative; z-index:999">
    <div style="display: flex; gap: .5rem;">
    <div style="
    background-color: var(--nvai-main);
    color: #fff;
    display: flex;
    align-items: center;
    justify-content: center;
...
" class="reviews"><img width="18" height="18" src="https://img.icons8.com/sf-ultralight-filled/${nvaiMainColorNumbers}/25/star.png" alt="star">4.5</div>
<div style="
    background-color: #fff;
    color: var(--nvai-main);
    display: flex;
    ...
    border: 1px solid #ebebeb;
    box-shadow: rgba(0, 0, 0, 0.21) 0px 6px 11px -3px, rgba(0, 0, 0, 0.05) 0px 2px 5px -2px;
" class="local"><img width="16" height="16" src="https://img.icons8.com/material-outlined/${nvaiMainColorNumbers}/24/visit.png" alt="box--v1">0un</div>
</div>
<div style="
      ...
      try {
        i.forEach(((e, t) => {
          ...
          if (0 == r ? y.querySelector(".iscatalog").remove(): (y.setAttribute("catalog", !0), l++), y.querySelector(".local") && y.querySelector(".local").remove(), y.querySelector(".imageset") && y.querySelector(".imageset").remove(), d && c[t]) {
            ...
            e && e.setAttribute("style", "background-color: #ffd900ff;color: var(--nvai-dark);border-radius: 0.5em;padding: 0.25em 0.75em;font-size: 0.86em;font-weight: 800;letter-spacing: 0.01em;margin-left: 0.5em;display: flex;align-items: center;justify-content: center;text-align: center; ")
          }
          if ("listing" == listView) {
          ...
          if (c[t] && !c[t].querySelector('.nvai-ad-listinfo_widget') && f) {
...
function n(e) {
  ...
  let i = document.getElementsByClassName("nvai-ad-listinfo_widget");
  for (let e = 0;
  e < i.length;
...
setTimeout((function () {
  ...
  if (document.getElementById("nvai-smetrics-status")) {
  ...
  #nvai-spinner {
    ...
    border-radius: 50%;
    animation: nvai-spinner-spin 1s linear infinite;
  }

  @keyframes nvai-spinner-spin {
    to {
...
<div style="display:flex; flex-direction:row; align-items:center; justify-content:space-between; width:100%; gap:1rem; font-family: Montserrat, sans-serif; margin-bottom: 10px;">
    <div style="display:flex; flex-direction:row; align-items:center; gap:1.5rem;">
      <div id="nvai-catalog-filter-container" style="display:flex; flex-direction:row; align-items:center; gap:.5rem; background:#222; border:1px solid #383838; padding:.4rem; border-radius:8px;">
        <button type="button" data-filter="todos" class="nvai-catalog-filter-btn" style="padding: 0.5rem 1rem; border-radius: 6px; border: none; background:#FFE600; color:#111; font-weight:700; font-size: 0.9rem; cursor:pointer; transition: all .2s ease;">Todos</button>
        <button type="button" data-filter="filtrar" class="nvai-catalog-filter-btn" style="padding: 0.5rem 1rem; border-radius: 6px; border: none; background:#333; color:#eaeaea; font-weight:600; font-size: 0.9rem; cursor:pointer; transition: all .2s ease;">Catálogos</button>
        <button type="button" data-filter="ocultar" class="nvai-catalog-filter-btn" style="padding: 0.5rem 1rem; border-radius: 6px; border: none; background:#333; color:#eaeaea; font-weight:600; font-size: 0.9rem; cursor:pointer; transition: all .2s ease;">Ocultar catálogos</button>
      </div>
      <div id="nvai-smetrics-status" style="display:flex; flex-direction:row; align-items:center; min-width: 22px;">
        <div id="nvai-spinner"></div>
      </div>
    </div>
  ...
  const setupCatalogFilterButtons = lists => {
    const container = document.getElementById("nvai-catalog-filter-container");
    if (!container) return;
...
function o(e) {
  ...
  function n() {
    a.getElementsByTagName("span")[0].innerText = "Carregando...", fetch(`${nvaiProxy}https://api.mercadolibre.com/trends/MLB/${e}`, eaInit).then((e => e.json())).then((t => function (t) {
      let a = t, i = [];
      for (let e = 0;
      e < a.length;
  ...
  t?.insertAdjacentHTML("beforeend", '<div id="eacattrends" style="display: inline-block;margin-bottom: 21px;"><span id="eacattrendsbtn" style="position:relative;z-index:3;font-weight:700;background-color:var(--nvai-main);color:#fff;padding:0.35em 0.75em;border-radius:7px;margin: 0em 0.5em;cursor: pointer;"><img src="https://img.icons8.com/ios-glyphs/60/ffffff/hot-sales-hours.png" style="width: 1.21em;position: relative;top: 3px;">\n            <span>Termos mais buscados! </span> <span style="font-size: 0.7em;position: relative;top: -2px;right: -3px;padding: 0px 5px 1px 5px;margin: 0px 0px 0px 5px;border: 1px solid #fff;border-radius: 1em;">categoria</span></span></div>'), t.setAttribute("style", "margin-bottom: -1.5em;");
...
async function l(n, a) {
  ...
  var s = "#7CFC00";
      function o(a, o, r) {
        ...
        , ((e, t) => m.add(t, "month"))), g = c.map((e => d(e.startOf("month")))), f = c.map((e => d(e.endOf("month")))), u = f.map(((e, t) => `${nvaiProxyLessRestricted}https://api.mercadolibre.com/items/visits?ids=${n.itemID}&date_from=${g[t]}&date_to=${e}`));
        ...
        (function () {
          t--, function () {
            function e() {
              ...
              e && (e.disabled = !1);
              let t = document.getElementById("nvai-catalog-filter-container");
              t && (t.style.display = "block");
              let n = document.getElementById("nvai-smetrics-status");
          ...
          ();
          let e = document.getElementById("nvai-smetrics-progress"), n = setInterval((function () {
            e = document.getElementById("nvai-smetrics-progress"), e && (e.value = e.value + 1, clearInterval(n))
          }
          ), 500)
        }())
      }
      let r = n.title.toLowerCase().replace(/[ãâàáäåāăąạảấầẩẫậắằẳẵặ]/g, "").replace(/[õôòóöøōŏőơọỏốồổỗộớờởỡợ]/g, "").replace(/[ñńņňṅṇṉṋṅ]/g, "").replace(/[ēĕėęěẹẻẽếềểễệ]/g, "").replace(/[īĭįỉịớờởỡợ]/g, "").replace(/[ūŭůűųụủứừửữự]/g, "").replace(/[ýỳỵỷỹ]/g, "").normalize("NFKD").replace(/[\u0300-\u036f]/g, "").split(" ").join("-").replace(/[^a-zA-Z0-9-]/g, "").replace(/^-/, ""), l = "https://produto.mercadolivre.com.br/MLB-" + n.itemID.split("MLB")[1], d = (n.catalogID.split("MLB")[1], n.itemID, i.getElementsByClassName("nvai-ad-listinfo_widget")[0]?.getAttribute("catalog"), l);
      if (i.setAttribute("product-id", n.itemID), i.setAttribute("product-price", n.price), i.setAttribute("shipping", n.shipping), "true" == i.getElementsByClassName("nvai-ad-listinfo_widget")[0]?.getAttribute("catalog") || n.catalogListed) {
...
" class="iscatalog">
  Catálogo
      ...
      ), !1, n.catalogListed ? nvaiProxy: null);
      else {
      ...
      (function () {
        ...
        e?.length > 0 && o(e), async function (e) {
          ...
          if (s.total_items_in_this_category && t) {
            t.innerHTML = `<b style=\"color: var(--nvai-main);font-size:18px;\">${s.total_items_in_this_category}</b> anúncios na categoria.`;
...
function runLogged() {
  eaOnAdminPanel || "https://www.mercadolivre.com.br/" != window.location.href && function (e) {
    ...
    a && (a.addEventListener("mouseover", (function () {
      ...
      if (e) e.style.opacity = "100%", e.style.display = "block", e.style.transform = "scale(1)", a.style.backgroundColor = "var(--nvai-main)";
      else {
        let e = "";
        ...
        if (s) {
          ...
          , t.addEventListener("mouseout", (function () {
            ...
            let e = document.getElementById("nvai-tool-modal");
            if (e) e.style.display = "block";
              ...
              document.getElementsByTagName("body")[0].insertAdjacentHTML("afterbegin", toolModal), document.getElementById("close-track").addEventListener("click", (function () {
                document.getElementById("nvai-tool-modal").style.display = "none"
              }
              ));
        ...
        l.innerHTML = "lite" == verif ? "Ativar Licença": `Conta ${verif.charAt(0).toUpperCase()}${verif.slice(1)}<img style="width:1rem" src="https://img.icons8.com/${nvaiMainColorNumbers}/ios-glyphs/30/guarantee--v1.png">`, l.addEventListener("mouseover", (function () {
          this.style.backgroundColor = "var(--nvai-main)", this.style.color = "#fff", this.style.fontWeight = "bold", this.innerHTML = "pro" == verif ? "Verificar": "Ativar"
        }
          ...
          if (this.style.backgroundColor = "var(--nvai-main)", this.style.color = "#fff", this.style.fontWeight = "bold", this.innerHTML = "Verificando conta...", "pro" != verif) {
            ...
            l.style.display = "none", this.insertAdjacentHTML("beforebegin", e), document.getElementById("validatebuy").addEventListener("click", (async function () {
              ...
              if (null == n || null == n || "" == n) t.style.border = "1px solid var(--nvai-main)";
              else {
                e.lastChild.innerHTML = "Verificando...";
                try {
                  await fetch(nvaiEndpoints.validate, {
                    method: "POST",
                    headers: {
          ...
          let e = document.getElementsByClassName("eamaindropdownmenu")[0];
          e.style.opacity = "100%", e.style.display = "block", e.style.transform = "scale(1)", a.style.backgroundColor = "var(--nvai-main)"
        }
        ))
      }
...
async function findUser() {
  if ("undefined" != sessionStorage.getItem("eauser") && null != sessionStorage.getItem("eauser")) {
    ...
    if ("loading" !== document.readyState) {
      ...
      a?.actual_track?.user?.user_id && (nvaiuser.id = a.actual_track.user.user_id), t = nvaiuser.email, usuario_logado = t, t != e && sessionStorage.setItem("eauser", t)
    }
    runLogged()
  }
    ...
    if (nvaiuser.id = n.actual_track.user.user_id, t = nvaiuser.email, usuario_logado = t, t && t != e && sessionStorage.setItem("eauser", t), null != usuario_logado) runLogged();
    else {
...
function nvaiStart() {
  ...
  if (null != nvai_version && null != nvai_version && "" != nvai_version || document.dispatchEvent(new CustomEvent("NovaiEvent", {
    detail: ""
  }
...
document.addEventListener("NovaiEvent", (function (e) {
  "" != e.detail && null != e.detail && null != e.detail && (nvai_version = e.detail, document.removeEventListener("NovaiEvent", (function (e) {})))
}
