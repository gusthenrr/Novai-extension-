...
function persistAuthState(e = {}) {
  ...
  const o = "number" == typeof i && i > 0 ? i: TTL1;
  if (a) {
    try {
  ...
  if ("string" == typeof t && t.trim()) {
    try {
      eadataStore(LOCAL_ACCESS_TOKEN_KEY, t, o)
    }
    catch (e) {}
    const e = eaHeaders && "function" == typeof eaHeaders.get,
    n = eaHeaders && "function" == typeof eaHeaders.set;
    let a = !1;
    try {
      a = !e || !eaHeaders.get("Authorization") || "background" === s
    }
    catch (e) {
      a = !0
    }
      if (a && n) try {
        eaHeaders.set("Authorization", " Bearer " + t)
    }
    catch (e) {}
  }
  if ("string" == typeof n && n.trim()) try {
    eadataStore(LOCAL_REFRESH_TOKEN_KEY, n, o)
  }
  catch (e) {}
}

document.addEventListener(AUTH_STATE_EVENT, (e => {
  persistAuthState(e.detail || {})
}));

function requestAuthStateFromBackground() {
  if (pendingAuthSyncPromise) return pendingAuthSyncPromise;
  return pendingAuthSyncPromise = new Promise((e => {
    const t = t => {
      clearTimeout(n), e(t?.detail || {})
    }
    ;
    const n = setTimeout((() => {
      document.removeEventListener(AUTH_STATE_EVENT, t), e(null)
    }
    ), 1e3);
    document.addEventListener(AUTH_STATE_EVENT, t, {
      once: !0
    }
    );
    try {
      ...
      clearTimeout(n), document.removeEventListener(AUTH_STATE_EVENT, t), e(null)
    }
  }
  )).finally((() => {
...
function broadcastAuthTokens(e, t) {
  if (!e && !t) return;
  try {
...
function isAccessTokenExpired(e) {
  try {
    const t = parseJwt(e);
    if (!t || !t.exp) return !1;
    const n = 1e3 * t.exp;
    return Date.now() + TOKEN_EXPIRY_SKEW_MS >= n
  }
  catch (e) {
    return !1
  }
...
async function getnewToken(e) {
  ...
  if (!t) {
    try {
      t = eadataRetrieve(LOCAL_REFRESH_TOKEN_KEY)
    }
    catch (e) {
      t = null
    }
  ...
  try {
    const e = await fetch("https://nossopoint-backend-flask-server.com/token_access", {
      method: "POST",
      headers: n,
      body: JSON.stringify({
        refresh_token: t
    ...
    if (a?.refresh_token) try {
      eadataStore(LOCAL_REFRESH_TOKEN_KEY, a.refresh_token, TTL1)
    }
    catch (e) {}
    if (e.ok && a?.access_token) return appendToken(a.access_token, a.refresh_token)
  }
  catch (e) {}
  return !1
}
...
function appendToken(e, t) {
  let n = e,
  a = t;
  if (e && "object" == typeof e && (n = e.access_token ?? e.token ?? e.accessToken, a = a ?? e.refresh_token ?? e.refreshToken), !n) return !1;
  try {
    eaHeaders.set("Authorization", " Bearer " + n)
  }
  catch (e) {}
  try {
    eadataStore(LOCAL_ACCESS_TOKEN_KEY, n, TTL1)
  }
  catch (e) {}
  if (a) try {
    eadataStore(LOCAL_REFRESH_TOKEN_KEY, a, TTL1)
  }
  catch (e) {}
  return broadcastAuthTokens(n, a), !0
}

async function ensureAuthorizationToken() {
  if (eaHeaders && "function" == typeof eaHeaders.get && eaHeaders.get("Authorization")) return !0;
  let e = getStoredAccessToken(),
  t = getStoredRefreshToken();
  if (!e && !t && (await requestAuthStateFromBackground(), e = getStoredAccessToken(), t = getStoredRefreshToken()), e && !isAccessTokenExpired(e)) return appendToken({
    access_token: e,
    refresh_token: t
  });
  if (t) {
    const e = await getnewToken(t);
    if (e) return e
  }
  return console.warn("NOVAI: Nenhum token de acesso válido encontrado. Conclua o login na extensão para habilitar as métricas."), !1
}
...
async function validateToken() {
  ...
  t = getStoredRefreshToken();
  async function n() {
    if (e && !isAccessTokenExpired(e)) return appendToken({
      access_token: e,
      refresh_token: t
    }), e;
    const n = await ensureAuthorizationToken();
    return n ? (e = getStoredAccessToken(), t = getStoredRefreshToken(), e): null
  }
  if (!e || isAccessTokenExpired(e)) {
    const a = await n();
    if (a) e = a;
    else {
      if (!tried) {
        tried = !0, await requestAuthStateFromBackground(), e = getStoredAccessToken(), t = getStoredRefreshToken();
        if (e && !isAccessTokenExpired(e)) {
          appendToken({
            access_token: e,
            refresh_token: t
      ...
      if (!e || isAccessTokenExpired(e)) return void askPermissions(usuario_logado)
    }
  }
  else appendToken({
    access_token: e,
  ...
  if (!e) return void askPermissions(usuario_logado);
  const a = new Headers;
  if (a.append("pragma", "no-cache"), a.append("cache-control", "no-cache"), a.append("Authorization", " Bearer " + e.toString()), AuthDataCheck("remote_user"), await new Promise((e => setTimeout(e, 500))), mfy_userdata?.remote?.email != usuario_logado) try {
    const a = await fetchUserMeData(e, !1);
    if (a.success && a.data) {
      const i = a.data;
      ...
      ), i.email == usuario_logado ? (appendToken({
        access_token: e,
        refresh_token: t
      }), dataCleanup()): askPermissions(usuario_logado)
    }
